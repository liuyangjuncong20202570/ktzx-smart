...
<template>
  <div class="experimental-practiceInfo-wrap">
      ...
      <el-form :model="practiceForm" label-width="auto" :inline="true" :rules="rules" ref="practiceFormRef">
        <el-form-item label="" prop="name">
          <div class="custom-form-item">
            ...
            <el-input class="custom-form-item-input" style="width: 200px" v-model.trim="practiceForm.name" :disabled="disable || !isEditable" />
          </div>
        </el-form-item>
    ...
    <el-table ref="multipleTableRef" class="custom-table" :data="tableData" style="width: 100%">
      ...
      <el-table-column label="名称" prop="itemName">
        <template #default="scope"> <span style="color: #ff4c48" v-if="scope.$index == 0 || scope.$index == 1">*</span> {{ scope.row.itemName }} </template>
      </el-table-column>
        ...
        <template #default="scope">
          <template v-if="scope.row.setWeight == 1">
            <el-input
              style="width: 80px"
              type="number"
              :disabled="disable || !canEditWeightAndCorrection"
      ...
      <el-table-column label="分数" prop="weight" width="150">
        <template #default="scope">
          <template v-if="scope.row.setWeight == 1">
            <el-input style="width: 100px" type="number" :disabled="true" :value="scope.row.score || calculateScore(scope.row.weight)" :min="0" :max="100" step-strictly /> 分
          </template>
          <template v-else></template>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" prop="address" width="350">
        <template #default="scope">
          <template v-if="scope.row.beValid == 1">
            <el-button v-if="scope.row.beDefault" class="custom-link-button custom-link-active-button" type="primary" :text="true" :disabled="!scope.row.defaultPath" @click="lookFile(scope.row)"
              >预览</el-button
            >
            <el-button
              ...
              type="primary"
              :text="true"
              :disabled="!scope.row.score || scope.row.score === 0 || scope.row.score % 1 !== 0 || !canEditWeightAndCorrection"
          ...
          <template v-if="scope.row.beValid == 1 && scope.$index < 2">
            <el-button
              v-if="scope.row.beDefault && scope.row.defaultPath"
              class="custom-link-button custom-link-active-button"
              type="primary"
              :text="true"
              ...
              >重新上传</el-button
            >
            <el-button v-else class="custom-link-button custom-link-active-button" type="primary" :text="true" @click="openUpload(scope.row)" :disabled="disable || !isEditable">上传</el-button>
          </template>
        </template>
      </el-table-column>
    </el-table>
  ...
  <!-- 选择kwa -->
  <el-dialog v-model="isOpenDialog" :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true" title="选择KWA" width="850" :before-close="beforeCloseDialog">
  ...
  <el-dialog v-model="isOpenLookDialog" :fullscreen="isFullscreen" :close-on-click-modal="false" :close-on-press-escape="false" :title="dialogTitle" :width="dialogWidth">
...
import { getTemplate, savePractice, practiceDetail, kwaTree, practiceUpdate, practiceUpdateChecking } from "@/api/practice/index.ts";
import { PracticeTemplateVO, ParamVO, DefaultParamVO, KwaVO, KwaParamVO, ResVO, UpdateCheckingVO } from "@/api/practice/type.ts";
...
const rules = reactive<FormRules<RuleForm>>({
  name: [{ required: true, message: "请输入实验名称", trigger: ["blur", "change"] }],
  totalScore: [
...
// 添加实验状态控制
const experimentStatus = ref<number>(Number(route.query.status) || 0);
// 根据实验状态判断是否允许编辑
const isEditable = computed(() => {
  // status = 0: 未发布 - 允许全部编辑
  // status = 1: 已发布 - 只允许修改权重和批改设定
  // status = 3: 停止 - 只允许修改权重和批改设定
  return experimentStatus.value === 0;
});
// 是否允许修改权重和批改设定
const canEditWeightAndCorrection = computed(() => {
  return experimentStatus.value === 0 || experimentStatus.value === 1 || experimentStatus.value === 3;
});
...
// 构建保存参数
const buildSaveParams = (): DefaultParamVO => {
  let arr: ParamVO[] = [];
...
// 执行保存操作
const executeSave = async (defaultParam: DefaultParamVO) => {
  let resData: ResVO = {};
  if (isDetail.value) {
    //编辑调用更新接口
  ...
  if (resData.code == 200) {
    const successMessage = experimentStatus.value === 0 ? "保存成功！" : "权重和批改设定保存成功！";
...
// 处理校验结果
const handleCheckingResult = async (checkingResult: UpdateCheckingVO, defaultParam: DefaultParamVO) => {
  const { couldUpdate, msg } = checkingResult.data;

  if (!couldUpdate) {
    // 不可修改，显示错误信息
    ElMessageBox.alert(msg || "当前修改不被允许", "修改被拒绝", {
  ...
  if (msg) {
    // 可以修改但有风险提示
    try {
      await ElMessageBox.confirm(msg, "确认修改", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        type: "warning",
      });
      // 用户点击确认，继续保存
      await executeSave(defaultParam);
    } catch {
      // 用户点击取消，不执行保存
      return;
    }
...
const saveData = async (formEl: FormInstance | undefined) => {
  // 根据实验状态显示不同的确认信息
  const confirmMessage = experimentStatus.value === 0 ? "确定保存吗?" : "确定保存权重和批改设定的修改吗?";
    ...
    .then(async () => {
      ...
      await formEl.validate(async (valid, fields) => {
        if (valid) {
          ...
          // 构建保存参数
          const defaultParam = buildSaveParams();
          // 如果是编辑模式，先调用校验接口

          if (isDetail.value) {
            try {
              const checkingResult: UpdateCheckingVO = await practiceUpdateChecking(defaultParam);
              if (checkingResult.code === "200") {
                await handleCheckingResult(checkingResult, defaultParam);
              } else {
