  ...
  <div class="grade-input-container">
    ...
    <div class="grade-list">
      ...
      <div v-for="(item, index) in gradeItems" :key="index" class="grade-item">
        <el-radio v-model="selectedGradeIndex" :label="index" :disabled="item.disabled || disable" @change="handleItemChange(index)" class="custom-radio">
          <div class="grade-content">
            <span class="grade-label">{{ item.label }}</span>
            <div class="score-control">
              <el-button class="control-btn" @click.prevent.stop="handleItemPlus(index)" :disabled="disable" :class="{ 'inactive-btn': selectedGradeIndex !== index }">
              ...
              />
              <el-button class="control-btn" @click.prevent.stop="handleItemMinus(index)" :disabled="disable" :class="{ 'inactive-btn': selectedGradeIndex !== index }">
    ...
    <el-dialog v-model="dialogVisible" class="edit-img-dialog" title="实验报告" width="1400" :before-close="handleClose" fullscreen style="overflow: hidden">
...
onMounted(() => {
  initScoreItems();
  // 只在组件挂载且有数据时初始化
  if (props.scoreCfg.mode === "percent" && props.scoreCfg.map && Object.keys(props.scoreCfg.map).length > 0) {
    initGradeItems();
  }
});
...
const updateDataItem = () => {
  ...
  props.dataObj.scoreCfg = {
    ...
    map: scoreMap || null,
    checked: (gradeItems.value[selectedGradeIndex.value] && gradeItems.value[selectedGradeIndex.value].label) || null,
  };
  props.dataObj.score = totalScore.value;
  props.dataObj.other ? props.dataObj.other : (props.dataObj.other = {});
  props.dataObj.other.score = totalScore.value;
