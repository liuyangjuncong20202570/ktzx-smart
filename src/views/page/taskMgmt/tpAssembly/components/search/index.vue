<template>
  ...
  <el-dialog class="search" v-model="dialogVisible" title="搜索" width="80%">
    <el-form ref="ruleFormRef" :model="ruleForm" label-width="auto" class="demo-ruleForm" :size="formSize" status-icon>
    ...
    <el-tabs type="border-card">
      <el-tab-pane label="搜索结果">
        <el-table ref="multipleTableRef" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" @select="handleSeachSelect" @select-all="handleSeachAllSelect">
...
const handleSeachAllSelect = (val) => {
  ...
  // 去掉id的项
  if (val.length == 0) {
    const cancelSelection = multipleSelection.value;
    console.log("我是取消cancelSelection", cancelSelection);
    // 去掉id的项
    selectedTableData.value = selectedTableData.value.filter((item) => !cancelSelection.some((cancelItem) => cancelItem.id === item.id));
    console.log("selectedTableData.value", selectedTableData.value);
  }
...
const handleSelectionChange = (val) => {
  return;
  console.log("我是selectionChange", val);
  multipleSelection.value = val;
  // 已选
  const selectIds = selectedTableData.value.map((item) => item.id);
  multipleSelection.value.forEach((item) => {
