...
const getPeopleList=()=>{
  request.admin.get('/sysmangt/personnelmangt/person?obsid='+currentobsid.value+'&catelog='+activeTab.value)
      .then(res => {
        if (res.code === 200) {
...
const deleteSelected = () => {
  ...
  request.admin.post('/sysmangt/personnelmangt/delete', ids)
      .then(res => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
