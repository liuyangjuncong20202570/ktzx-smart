  ...
  <el-container style="height: 92vh">
    ...
    <el-main style="padding: 0; overflow: auto">
      ...
      <el-table
        ...
        :default-sort="{ prop: 'procode', order: 'ascending' }"
        stripe
      >
        ...
        <el-table-column prop="proname" label="专业名称" min-width="100">
          <template #default="{ row }">
            <el-input
              v-if="row.editingProname"
              :ref="el => setInputRef(el, row)"
              style="width: 100%; height: 25px"
              v-model="row.proname"
              @blur="handleBlur(row, 'editingProname')"
            ></el-input>
            <div
              v-else
              style="width: 100%; height: 25px"
              @click="handleClick(row, 'editingProname')"
            >
              {{ row.proname }}
            </div>
          </template>
        ...
        <el-table-column prop="procode" label="专业代码" min-width="80">
          <template #default="{ row }">
            <el-input
              v-if="row.editingProcode"
              :ref="el => setInputRef(el, row)"
              style="width: 100%; height: 25px"
              v-model="row.procode"
              @blur="handleBlur(row, 'editingProcode')"
            ></el-input>
            <div
              v-else
              style="width: 100%; height: 25px"
              @click="handleClick(row, 'editingProcode')"
            >
              {{ row.procode }}
            </div>
          </template>
        ...
        <el-table-column prop="reachpercent" label="课程目标达成阈值" min-width="80">
          <template #default="{ row }">
            <el-input
              v-if="row.editingReachpercent"
              :ref="el => setInputRef(el, row)"
              style="width: 100%; height: 25px"
              v-model="row.reachpercent"
              @blur="handleBlur(row, 'editingReachpercent')"
            ></el-input>
            <div
              v-else
              style="width: 100%; height: 25px"
              @click="handleClick(row, 'editingReachpercent')"
            >
              {{ row.reachpercent }}
            </div>
          </template>
...
const handleRoleAdd = () => {
  ...
  request.admin
    .post('/sysmangt/professionmangt/create', newCollege.value)
    .then(res => {
...
const handleBlur = (row, field) => {
  nextTick(() => {
    ...
    if (hasChanged) {
      ...
      request.admin
        .post('/sysmangt/professionmangt/update', updateItem.value)
        .then(res => {
          // 登录成功
          if (res.code === 200) {
            ElMessage({
  ...
  if (selectedRows.value.length === 0) {
    ...
    ElMessageBox.confirm('选中的角色将被删除，是否确定', '警告', {
      ...
      .then(() => {
        ...
        request.admin
          .post('/sysmangt/professionmangt/delete', deleteIdList.value)
          .then(res => {
            // 登录成功
            if (res.code === 200) {
              ElMessage({
