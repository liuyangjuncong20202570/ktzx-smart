<template>
  <el-container style="height: 92vh;">
    ...
    <el-main style="padding: 0;overflow: auto;">
      <!--生成-->
      <el-table :data="tableData" style="height: 100%; table-layout:auto; width: 100%;" @select-all="handleSelectAll"
        @selection-change="handleSelectionChange" :default-sort="{ prop: 'procode', order: 'ascending' }" stripe>
        ...
        <el-table-column prop="proname" label="专业名称" min-width="100">
          <template #default="{ row }">
            <el-input v-if="row.editingProname" :ref="el => setInputRef(el, row)" style="width: 100%; height: 25px;"
              v-model="row.proname" @blur="handleBlur(row, 'editingProname')"></el-input>
        ...
        <el-table-column prop="procode" label="专业代码" min-width="80">
          <template #default="{ row }">
            <el-input v-if="row.editingProcode" :ref="el => setInputRef(el, row)" style="width: 100%; height: 25px;"
              v-model="row.procode" @blur="handleBlur(row, 'editingProcode')"></el-input>
        ...
        <el-table-column prop="reachpercent" label="课程目标达成阈值" min-width="80">
          <template #default="{ row }">
            <el-input v-if="row.editingReachpercent" :ref="el => setInputRef(el, row)"
              style="width: 100%; height: 25px;" v-model="row.reachpercent"
