<template>
  <el-container
    style="height: 92vh; width: 100%; text-align: left; background-color: white; color: #000000"
  >
    <div class="cover"></div>
    <el-header
      style="
        ...
        <el-form-item>
          <el-table :data="form.stLevelList" style="width: 100%; height: 334px" border stripe>
            ...
            <el-table-column label="级别" align="center">
              <template v-slot="row">
                <el-input
                  v-if="row.row.editingLevelName"
                  :ref="el => setInputRef(el, row.row)"
                  v-model="row.row.levelname"
            ...
            <el-table-column label="是否启用" align="center">
              <template v-slot="row">
                <el-switch
                  v-model="row.row.obsdeep"
                  class="ml-2"
                  @change="changeObsdeep(row.row)"
            ...
            <el-table-column label="教师所在级别" align="center">
              <template v-slot="row">
                <el-checkbox
                  v-model="row.row.teacher"
                  @change="changeTeacherLevel(row.row)"
                  :disabled="!row.row.obsdeep"
                ></el-checkbox>
              </template>
            </el-table-column>
            <el-table-column label="学生所在级别" align="center">
              <template v-slot="row">
                <el-checkbox
                  v-model="row.row.student"
                  @change="changeStudentLevel(row.row)"
                  :disabled="!row.row.obsdeep"
                ></el-checkbox>
              </template>
            </el-table-column>
...
const changeSchoolConfig = postData => {
  request.admin
    .post('/sysmangt/schoolmangt/update', postData)
    .then(res => {
      if (res.code === 200) {
        getData();
      } else {
...
const changeObsnameOrRemark = postData => {
  request.admin
    .post('/sysmangt/schoolmangt', postData)
    .then(res => {
      if (res.code === 200) {
        getData();
...
const changeTeacherLevel = row => {
  ...
  if (selectedTeacherLevel.value) {
    // 只能单选一个所在级别
    if (selectedTeacherLevel.value === row) {
      // 不允许取消勾选
      row.teacher = true;
      return;
    }
...
const changeStudentLevel = row => {
  // console.log(form.value.stLevelList)
  if (selectedStudentLevel.value) {
    // 只能单选一个所在级别
    if (selectedStudentLevel.value === row) {
      // 不允许取消勾选
      row.student = true;
      return;
    }
