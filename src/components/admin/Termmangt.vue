    ...
    <el-main style="padding: 0;">
      <div style="max-height: 100%; height: 100%; overflow:auto ; flex: auto">
        <el-table :data="tableData" style="table-layout:auto; width: 100%; height: 100%;" @select="handleSelect"
          @selection-change="handleSelectionChange" :default-sort="{ prop: 'enddate', order: 'descending' }"
          class="my-custom-table" stripe>
          ...
          <el-table-column prop="termname" label="学期" min-width="130">
            <template #default="{ row }">
              <el-input v-if="row.editingTermName" :ref="el => setInputRef(el, row)" style="width:100%; height: 25px;"
                v-model="row.termname" @blur="handleBlur(row, 'editingTermName')"></el-input>
          ...
          <el-table-column prop="begindate" label="起始日期" min-width="120">
            <template #default="{ row }">
              <el-date-picker v-if="row.editingbegindate" :ref="el => setInputRef(el, row)"
                style="width: 100%; height: 25px;" v-model="row.begindate" @blur="handleBlur(row, 'editingbegindate')"
          ...
          <el-table-column prop="enddate" label="结束日期" min-width="120">
            <template #default="{ row }">
              <el-date-picker v-if="row.editingEndDate" :ref="el => setInputRef(el, row)"
                style="width: 100%; height: 25px;" v-model="row.enddate" @blur="handleBlur(row, 'editingEndDate')"
                placeholder="请选择日期" value-format="YYYY-MM-DD"></el-date-picker>
          ...
          <el-table-column prop="remark" label="备注" min-width="200">
            <template #default="{ row }">
              <el-input v-if="row.editingRemark" :ref="el => setInputRef(el, row)" style="width: 100%; height: 25px;"
                v-model="row.remark" @blur="handleBlur(row, 'editingRemark')"></el-input>
          ...
          <el-table-column label="当前学期" min-width="200">
            <template #default="{ row }">
              ...
              <el-checkbox :key="row.iscurrenttermBoolean" v-model:checked="row.iscurrenttermBoolean"
