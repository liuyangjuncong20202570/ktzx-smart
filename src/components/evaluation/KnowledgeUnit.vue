<template>
  <el-container style="height: 92vh;">
    <!--两个按钮，靠最左-->
    <el-header style="height: auto; padding: 5px 0px; width:100%; background-color:#deebf7; display: flex; align-items: center;">
    ...
    <el-main style="padding: 0; overflow: auto; background-color: white;">
      <el-table class="uniqueTable" ref="sortableInstance" :data="tableData" style="width: 100%;margin-bottom: 20px;" row-key="id" highlight-current-row
                size="large" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :row-class-name="tableRowClassName"
...
<!--          </template>-->
        ...
        <el-table-column prop="datavalue" label="数值" sortable width="100">
          <template #default="tableRowData">
            ...
            <el-input ref="inputDataValueRef" v-else v-model="tableRowData.row.datavalue" @blur="saveEditRef(tableRowData.row,'editDataValue')"
                      @keyup.enter="saveEditRef(tableRowData.row,'editDataValue')"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="kwas" label="基本教学目标">
          <template #default="tableRowData">
            <span v-if="!tableRowData.row.isChildren">{{ tableRowData.row.tempKwas.map(item => item.name).join(", ") }}</span>
    ...
    onEnd: (evt) => {
      ...
      if(oldIndex !== newIndex){
        ...
        if(selData.pOrderNum === toData.pOrderNum){
          ...
          request.evaluation.post('/evaluation/knowledgeUnit/updateKnowledgeUnitOrdernum?preOrdernum=' + (toData.ordernum-1), data).then((res) => {
            if(res.code === 200){
