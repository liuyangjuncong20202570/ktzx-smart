    ...
    <el-main style="padding: 0">
      ...
      >
        ...
        <el-table-column prop="datavalue" label="权值">
          <template v-slot="row">
            <el-input
              v-if="row.row.editingDatavalue"
              style="width: 100%; height: 25px"
              v-model="row.row.datavalue"
              :ref="el => setInputRef(el, row)"
              @blur="handleBlur(row, 'editingDatavalue')"
            ></el-input>
            <div
              v-else
              style="width: 100%; height: 25px"
              @dblclick="handleClick(row, 'editingDatavalue')"
            >
              {{ row.row.datavalue }}
            </div>
          </template>
...
const deleteKWA = () => {
  ...
  ElMessageBox.confirm('选中的KWA将被删除，是否确定', '警告', {
    ...
    .then(() => {
      ...
      request.evaluation
        .post('/evaluation/kwadict/delete', deletedIds)
        .then(res => {
          if (res.code === 200) {
            getKWAData();
...
const createNewKWA = data => {
  request.evaluation
    .post('/evaluation/kwadict/create', data)
    .then(res => {
      if (res.code === 200) {
        getKWAData();
