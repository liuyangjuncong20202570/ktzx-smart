<template>
	<el-container style="height: 92vh;">
		...
		<el-main style="padding: 0; overflow: auto; background-color: white;">
			<el-table :data="tableData" style="width: 100%;" :cell-style="cellItemNameStyle"
				:header-cell-style="rowStyle">
				<el-table-column align="center" prop="name" label="课程目标" width="300">
				</el-table-column>
				<el-table-column v-for="(item, index) in tableColumns" align="center" :prop="item.id"
					:label="item.itemName" :key="index">

					<template #default="tableRowData">
						<el-table-column v-if="item.childrens && item.childrens.length > 0"
							v-for="(child1, childIndex1) in item.childrens" align="center" :prop="child1.id"
							:label="child1.itemName" :key="childIndex1">

							<template #default="tableRowData">
								<el-table-column v-if="child1.childrens && child1.childrens.length > 0"
									v-for="(child2, childIndex2) in child1.childrens" align="center" :prop="child2.id"
									:label="child2.itemName" :key="childIndex2">
								</el-table-column>
								<div v-else>
									<span v-if="editRef.get(tableRowData.row.id)[child1.id] === undefined"
										@dblclick="editEditRef(tableRowData.row, child1.id)">{{
											tableRowData.row[child1.id] }}</span>
									<el-input :ref="el => inputRefs.set(child1.id, el)" v-else
										v-model="tableRowData.row[child1.id]"
						...
						<div v-else>
							<span v-if="editRef.get(tableRowData.row.id)[item.id] === undefined"
								@dblclick="editEditRef(tableRowData.row, item.id)">{{ tableRowData.row[item.id]
								}}</span>
				...
				<template #append>
					<div style="text-align: center">
						<el-table :data="tableAppendDataFirst" style="width: 100%;"
							:cell-style="{ 'background-color': '#eaeaea', color: '#000000', 'border-color': '#bdbdbd' }"
								...
								<template #default="tableRowData">
										...
										<template #default="tableRowData">
											<div>
												<span v-if="editRef.get('percent')[child1.id] === undefined"
													@dblclick="editEditRef('percent', child1.id)">{{
														tableRowData.row[child1.id] }}</span>
												<el-input :ref="el => inputRefs.set('percent' + child1.id, el)" v-else
													v-model="tableRowData.row[child1.id]"
									...
									<div v-else>
										<span v-if="editRef.get('percent')[item.id] === undefined"
											@dblclick="editEditRef('percent', item.id)">{{
												tableRowData.row[item.id] }}</span>
										<el-input :ref="el => inputRefs.set('percent' + item.id, el)" v-else
											v-model="tableRowData.row[item.id]"
