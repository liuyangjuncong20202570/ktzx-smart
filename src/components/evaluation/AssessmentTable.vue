	...
	<div>
		...
		<div id="container" style="height: calc(92vh - 130px); width: 100%;">
			<vxe-grid v-bind="gridOptions" v-on="gridEvents" :edit-config="gridOptions.editConfig"
				class="scroll-container" @edit-closed="handleEditClosed"></vxe-grid>
		</div>
		...
		<el-dialog v-model="deleteDialogVisible" width="350" destroy-on-close>
			...
			<div v-if="associateFiles.length"
				style="max-height: 300px; overflow: auto; display: flex; flex-direction: column; align-items: center">
...
const gridOptions = ref({
	...
	menuConfig: {
		...
		visibleMethod({ options, column }) {		// 处理右键菜单项显示
			rightClickItem.value = column;
			let isDisabled = true;		// 菜单项是否不可可点击
			let isVisible = false;		// 菜单项是否可见
			if (info.value.head.some((item) => leafIds.value[item.id].includes(column.field))) {	// 只有子节点才能触发右键菜单
				isDisabled = false;
				isVisible = true;
			}
...
const creatHeader = (head, floor = 0, ancestorId = '') => {
	...
	head.forEach((node, idx) => {
		...
		if (node.children && node.children.length > 0) creatHeader(node.children, floor + 1, ancestorId);
		else {		// 到达最深层节点（叶节点）
			...
			node.width = Number(
				(
					(
						(container.value.clientWidth - 240) / info.value.head.length
...
const save = async () => {
	...
	if (!info.value.head.every((item) => footer1Data.value[0][item.id] === 100) || calcFooter2Data() !== 100) {
...
const getFileList = async () => {
	try {
		const res = await request.evaluation.get(`/evaluation/assessmentPlan/getFileList?checkitemId=${rightClickItem.value.field}`);
		if (res.code === 200) {
...
const associate = async () => {
	...
	if (newFiles.length) {
		...
		try {
			const res = await request.evaluation.post(`/evaluation/assessmentPlan/associate`, associatePost);
			if (res.code === 200) {
				getFileList();
			} else {
				ElMessage.error(res.msg);
	...
	if (deletedFiles.length) {
		...
		try {
			const res = await request.evaluation.post(`/evaluation/assessmentPlan/disassociate`, disassociatePost);
			if (res.code === 200) {
				getFileList();
			} else {
				ElMessage.error(res.msg);
...
const openDeleteDialog = async (row) => {
	...
	try {
		const res = await request.evaluation.get(`/evaluation/assessmentPlan/getAssociateCheckitems?fileId=${filesTableRowData.value.id}`);
		if (res.code === 200) {
