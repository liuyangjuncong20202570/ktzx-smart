...
console.log('pathHeader------------', pathHeader + '/exp/myExpProgram');
const routes = [
  {
    path: '/login',
    name: 'Login',
    // component: () => import('../views/Login.vue')
    component: () => import('../views/RedrawPage/Login.vue')
  },
  {
    path: '/homes/studenthome',
    name: 'StudentHomePage',
    component: () => import('../views/RedrawPage/studentHomePage.vue')
  },
  {
    path: '/homes/studentcourses',
    name: 'StudentCoursePage',
    // component: () => import('../views/StudentCoursePage.vue'),
    component: () => import('../views/RedrawPage/studentCoursePage.vue'),
    children: [
      // 学生端我的作业
      {
  ...
  },
  {
    ...
    children: [
      ...
      {
        // /coursemangt/classroommangt/classStudentList
      ...
      {
        path: 'exp/experimentPage', // 实验主页
        component: () => import('../components/course/experimentPage.vue')
      },
      {
        path: 'exp/teachingmangt', // 实验课程大纲
        component: () => import('../components/course/InstructionalProgrammangt.vue')
      },
      {
        path: 'exp/clsRoomList', // 实验课程学生名单
        component: () => import('../components/course/Classroommangt/StudentsList.vue')
      },
      {
        path: 'exp/assignedAssistant', // 指定助教
        component: () => import('../components/course/AssignedAssistant.vue')
      },
      // 问卷
      {
...
router.beforeEach((to, from, next) => {
  ...
  // 如果当前路由与选中的路由不一致，则根据 URL 跳转
  console.log(to);
  if (to.query.courseChineseName) {
    console.log('test');
    sessionStorage.setItem('courseChineseName', JSON.stringify(to.query.courseChineseName));
  }
  if (
