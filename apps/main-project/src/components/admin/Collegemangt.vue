...
const handleRoleAdd = () => {
  ...
  request.admin
    .post('/sysmangt/collegemangt', newCollege.value)
    .then(res => {
      // 登录成功
      if (res.code === 200) {
        ElMessage({
...
const handleBlur = (row, field) => {
  nextTick(() => {
    ...
    if (hasChanged) {
      ...
      request.admin
        .post('/sysmangt/collegemangt/update', updateItem.value)
        .then(res => {
          // 登录成功
          if (res.code === 200) {
            ElMessage({
  ...
  if (selectedRows.value.length === 0) {
    ...
    ElMessageBox.confirm('选中的角色将被删除，是否确定', '警告', {
      ...
      .then(() => {
        ...
        request.admin
          .post('/sysmangt/collegemangt/delete', deleteIdList.value)
          .then(res => {
            // 登录成功
            if (res.code === 200) {
              ElMessage({
