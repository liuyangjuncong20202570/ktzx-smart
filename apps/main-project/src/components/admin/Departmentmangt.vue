<template>
  <Header style="text-align: center; border-bottom: 2px solid #27a5ff" title="专业管理" />
  <el-container style="height: 92vh">
    ...
    <el-main style="padding: 0; overflow: auto">
      ...
      <el-table
        class="custom-table"
        :data="tableData"
        style="height: 100%; table-layout: auto; width: 100%; font-weight: bold"
        @select-all="handleSelectAll"
        @selection-change="handleSelectionChange"
        :default-sort="{ prop: 'levelcode', order: 'ascending' }"
        stripe
      >
        ...
        <el-table-column prop="obsname" label="部门名称" min-width="100">
          <template #default="{ row }">
            <el-input
              v-if="row.editingobsname"
              :ref="el => setInputRef(el, row)"
              style="width: 100%; height: 25px"
              v-model="row.obsname"
              @blur="handleBlur(row, 'editingobsname')"
            ></el-input>
            <div
              v-else
              style="width: 100%; height: 25px"
              @click="handleClick(row, 'editingobsname')"
            >
              {{ row.obsname }}
            </div>
          </template>
        ...
        <el-table-column prop="levelcode" label="层级码" min-width="80">
          <template #default="{ row }">
            <div style="width: 100%; height: 25px" @click="handleClick(row, 'editingLevelcode')">
              {{ row.levelcode }}
            </div>
          </template>
        </el-table-column>
...
const handleRoleAdd = () => {
  ...
  request.admin
    .post('/sysmangt/department/create', newCollege.value)
    .then(res => {
      // 登录成功
      if (res.code === 200) {
        ElMessage({
...
const handleBlur = (row, field) => {
  nextTick(() => {
    ...
    if (hasChanged) {
      ...
      request.admin
        .post('/sysmangt/department/update', updateItem.value)
        .then(res => {
          // 登录成功
          if (res.code === 200) {
            ElMessage({
  ...
  if (selectedRows.value.length === 0) {
    ...
    ElMessageBox.confirm('选中的角色将被删除，是否确定', '警告', {
      ...
      .then(() => {
        ...
        request.admin
          .post('/sysmangt/department/delete', deleteIdList.value)
          .then(res => {
            // 登录成功
            if (res.code === 200) {
              ElMessage({
