...
<template>
  <div class="wrapper">
    ...
    <div class="text">
      当前展示第{{ currentTimes }}次评价，已评价{{ classroomGraphStore.totalTimes }}次
    </div>
    <div
...
const currentTimes = ref(classroomGraphStore.totalTimes);
...
const getData = async () => {
  try {
    const nodesCombosRes = await request.evaluation.get(
    ...
    if (nodesCombosRes.code === 200) {
      ...
      ElMessage.error(nodesCombosRes.msg);
    }
  } catch (error) {
...
const getNodesCombosProp = (data, parent = null) => {
  data.forEach(item => {
    ...
    item.kwas.forEach(node => {
      ...
      nodesMap.set(
        JSON.stringify([node.unitid, node.kwaid]),
        nodes.value[nodes.value.length - 1].id
      );
    });
...
onMounted(async () => {
  ...
  timeLineOption.value = {
    baseOption: {
      timeline: {
        ...
        data: classroomGraphStore.charts[0].timelineData,
        currentIndex: classroomGraphStore.totalTimes,
        checkpointStyle: {
  ...
  const contextMenu = new G6.Menu({
    ...
    handleMenuClick: (target, item) => {
      ...
      request.evaluation
        .post('/evaluation/knowledgeUnit/updateKnowledgeUnitKwa', postData)
        .then(res => {
          if (res.code === 200) {
